"""Raise the FR3's collision thresholds for contact-rich bulb screwing.
...
Bulb screwing is contact-rich by design -- the jaws squeeze the glass, the bulb
resists as it is lifted, and the threads load the wrist while turning -- so the
defaults fire constantly on forces that are the task working correctly, not a
collision.
...
class CollisionBehaviorNode(Node):
    def __init__(self):
        super().__init__("bulbslot2d_collision_behavior")
        defaults = {
            ...
            # Cartesian (6, forces then torques).
    ...
    def _on_result(self, future):
        ...
        if result is not None and result.success:
            self.get_logger().info(
                ...
                "the arm now tolerates grip and thread forces without a reflex")
        else:
