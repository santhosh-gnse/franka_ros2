"""PS5 teleoperation front-end for the 2-D bulb-slotting pipeline.

Publishes normalized 3-D actions for franka_bulbslot2d; it never talks to
Servo directly.

    action[0]   velocity along the home-seat line ("Y'"), toward/away from the socket
    action[1]   vertical velocity (Z), up/down
    action[2]   gripper, -1 fully closed .. +1 fully open

This is franka_bulbscrew's ps5_teleop_node.py trimmed from 5-D to 3-D: the
home (bulb-in-hand) pose and the socket seat pose are two fixed, already-
calibrated points, and the horizontal direction between them plus world
vertical span every motion this task needs -- see safety_node.py for the
plane math and franka_bulbslot2d/README.md for the derivation. There is no
commanded wrist yaw at all; safety_node holds the full orientation and the
out-of-plane axis instead.

Controller mapping:

    L1 (4)          dead-man; nothing moves unless held
    left stick Y    vertical (Z), up/down
    left stick X    motion along the home-seat line (Y')
    Cross / Triangle  close / open the gripper (latched, not momentary)
    PS (10)         home
    Circle (1)      start episode (homes first)
    Square (3)      stop episode (saves, then homes)

The whole task is one plane, so the left stick alone commands both DOF at
once -- push it in any direction and the tool moves in the corresponding
combination of "toward/away from the socket" and "up/down", the way a
top-down 2-D game stick would. L2/R2 and the right stick are unused -- there
is no out-of-plane or yaw degree of freedom to command in this task.

The gripper is latched rather than proportional because a bulb needs a
settled grip: a stick axis would jitter the commanded width every tick and
franka_gripper would keep re-issuing goals. Latching also means releasing the
dead-man does not drop the bulb -- safety_node keeps publishing the last
width.
"""
...
JOY_TOPIC = "/joy"
ACTION_TOPIC = "/bulbslot2d/teleop_action"
DEADMAN_TOPIC = "/bulbslot2d/deadman"
PUBLISH_RATE = 20.0

ENABLE_BUTTON = 4         # L1
HOME_BUTTON = 10          # PS
START_COLLECT_BUTTON = 1  # Circle
STOP_COLLECT_BUTTON = 3   # Square
CLOSE_BUTTON = 0          # Cross
OPEN_BUTTON = 2           # Triangle
AXIS_LEFT_X = 0
AXIS_LEFT_Y = 1
# Sign of each plane axis. Flip if the arm moves opposite the stick.
SIGN_YPRIME = -1.0
SIGN_Z = 1.0
DEADZONE = 0.05
JOY_TIMEOUT_S = 0.15
...
# Same home pose as franka_bulbscrew, UNCHANGED -- this package does not
# remeasure anything, only reduces the action space between the same two
# points. fr3_hand_tcp = [0.7369, -0.1828, 0.2096] in fr3_link0, tool
# vertical, 81 mm directly above the bulb's grasp point when the bulb stands
# on the plank. Must stay equal to servo_ik_node's nullspace_target.
HOME_JOINTS = {
    "fr3_joint1": -0.288396,
    "fr3_joint2": 0.807440,
    "fr3_joint3": 0.070228,
...
START_COLLECT_SRV = "/bulbslot2d/start_episode"
STOP_COLLECT_SRV = "/bulbslot2d/finish_episode"
SERVO_SETTLE_S = 0.2
SWITCH_COMMAND_TYPE_SRV = "/servo_node/switch_command_type"


class Bulbslot2dTeleop(Node):
    def __init__(self):
        super().__init__("bulbslot2d_ps5_teleop")
    ...
    def _on_joy(self, msg):
        ...
        self._edge(msg, CLOSE_BUTTON, lambda: setattr(self, "_grip_open", False))  # Cross
        self._edge(msg, OPEN_BUTTON, lambda: setattr(self, "_grip_open", True))    # Triangle
    ...
    def _on_timer(self):
        ...
        action = [0.0, 0.0, 1.0 if self._grip_open else -1.0]
        if enabled and max(AXIS_LEFT_X, AXIS_LEFT_Y) < len(joy.axes):
            s_yprime = SIGN_YPRIME * float(joy.axes[AXIS_LEFT_X])
            s_z = SIGN_Z * float(joy.axes[AXIS_LEFT_Y])
            # One combined deadzone on the whole stick, not two independent
            # per-axis ones: this is a single 2-D control now, matching
            # franka_pusht's ps5_teleop_node.py for the same reason -- a
            # per-axis deadzone would let a stick held slightly off a pure
            # axis silently drop the smaller component instead of commanding
            # the diagonal the operator actually pushed.
            if math.hypot(s_yprime, s_z) > DEADZONE:
                action[0], action[1] = s_yprime, s_z
    ...
    def _home_sequence(self):
        self._busy = True
        try:
            # Open the jaws first, matching franka_bulbscrew's convention. The
            # task starts with the bulb placed into the jaws at the home pose,
            # so arriving there closed means an extra step -- and homing while
    ...
    def _publish_stop_action(self):
        ...
        self._action_pub.publish(Float32MultiArray(
            data=[0.0, 0.0, 1.0 if self._grip_open else -1.0]))
