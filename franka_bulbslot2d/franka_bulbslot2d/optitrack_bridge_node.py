...
Republishes the tracked bulb (`bulb-trirl`) and the marker fixed to the robot's
stand. Poses pass through unchanged, so they stay in mocap4r2's Y-UP `map`
frame -- observation_node is what converts into the socket-seat frame.
"""
...
class OptitrackBridgeNode(Node):
    def __init__(self):
        super().__init__("bulbslot2d_optitrack_bridge")
        defaults = {
            ...
            "pole_base_pose_topic": "/bulbslot2d/pole_base_pose",
        }
        for name, value in defaults.items():
            self.declare_parameter(name, value)
        self.publishers_by_name = {
            self.get_parameter("bulb_rigid_body_name").value: self.create_publisher(
                PoseStamped, self.get_parameter("bulb_pose_topic").value, 10),
