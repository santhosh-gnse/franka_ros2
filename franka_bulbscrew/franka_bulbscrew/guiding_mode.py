...
interfaces, so exactly one may be active. This flips between them in one call.
    ...
    ros2 run franka_bulbscrew guiding_mode --ros-args -p enable:=true    # guide
    ros2 run franka_bulbscrew guiding_mode --ros-args -p enable:=false   # normal
...
class GuidingMode(Node):
    def __init__(self):
        super().__init__("bulbscrew_guiding_mode")
    ...
    def run(self):
        ...
        self.get_logger().info(
            "GUIDING MODE: the arm is free to move by hand. Recording still works "
            "(FCI is on). Run with enable:=false before homing or teleop."
            if enable else
