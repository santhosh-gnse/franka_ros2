"""Deployment shell for an exported deterministic bulb-screwing policy.

Loads a trirl_ppo_fb (trust-region-irl) policy checkpoint's params, exported
as a plain numpy .npz (see franka_bulbscrew/models/bulbscrew_best_policy.npz), and
...
-> elu -> Dense(5)), matching trust_region_irl/algorithms/trirl_ppo_fb/
flax_full_jit/policy.py. The policy operates on the raw 25-dim observation
...
the sim expert dataset) -- its actions assume the socket-seat frame and the action scales (max_linear_speed,
max_yaw_rate) match what the policy was trained with -- see SIM_ALIGNMENT.md.
...
class PolicyNode(Node):
    def __init__(self):
        super().__init__("bulbscrew_policy")
        default_model_path = os.path.join(
            get_package_share_directory("franka_bulbscrew"), "models", "bulbscrew_best_policy.npz")
        self.declare_parameter("observation_topic", "/bulbscrew/observation")
        self.declare_parameter("policy_action_topic", "/bulbscrew/policy_action")
    ...
    def _on_obs(self, msg):
        ...
        if self.weights is not None and obs.shape == (25,) and np.all(np.isfinite(obs)):
