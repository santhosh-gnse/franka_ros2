"""Record 20 Hz PushT transitions and save one NPZ file per episode."""
...
class DataCollectionNode(Node):
    def __init__(self):
        super().__init__("pusht_data_collection")
        defaults = {"control_rate_hz": 20.0, "observation_timeout_s": 0.15,
                    "action_timeout_s": 0.15, "observation_topic": "/pusht/observation",
                    "executed_action_topic": "/pusht/executed_action", "dataset_root": "~/pusht_data"}
        ...
        (self.session / "metadata.json").write_text(json.dumps({
            ...
            "observation_dim": 24, "action_dim": 2,
            "observation": "block_rel_goal_xyz + block_rel_goal_wxyz + tcp_rel_goal_xyz + q7 + dq7",
        }, indent=2))
        ...
        self.create_service(Trigger, "/pusht/start_episode", self._start)
        self.create_service(Trigger, "/pusht/finish_episode", self._finish)
        self.create_service(Trigger, "/pusht/abort_episode", self._abort)
    ...
    def _fresh(self):
        now = self._now()
        return (self.obs is not None and self.obs.shape == (24,) and self.action is not None and
                self.action.shape == (2,) and now-self.obs_s <= self.get_parameter("observation_timeout_s").value and
